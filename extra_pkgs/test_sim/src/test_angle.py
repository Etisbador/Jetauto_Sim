...
class PoseControl:
    def __init__(self):
        ...
        rospy.Subscriber('/odom', Odometry, self.odom_callback)		#se debe establecer la publicacion de la odometri a de gazebo
        ...
        if self.guardar_datos:
            ...
            directory = os.path.join(package_path,'datos','angle')
            if not os.path.exists(directory):
                os.makedirs(directory)  # Create the directory if it doesn't exist
            self.file_name = os.path.join(directory, f"Angle.txt")
    ...
    def odom_callback(self, msg):
        ...
        (roll, pitch, self.theta_odom) = euler_from_quaternion([rot_q.x, rot_q.y, rot_q.z, rot_q.w])
