...
class PoseControl:
    def __init__(self):
        ...
        self.A0x = self.x_kp + self.x_ki * self.Ts / 2.0 + 2.0 * self.x_kd / self.Ts
        self.A1x = -self.x_kp + self.x_ki * self.Ts / 2.0 - 2.0 * self.x_kd / self.Ts
        self.A2x = self.x_kd / self.Ts  # Additional term needed for proper derivative calculation
        
        self.A0y = self.y_kp + self.y_ki * self.Ts / 2.0 + 2.0 * self.y_kd / self.Ts
        self.A1y = -self.y_kp + self.y_ki * self.Ts / 2.0 - 2.0 * self.y_kd / self.Ts
        self.A2y = self.y_kd / self.Ts  # Additional term needed for proper derivative calculation
        
        self.A0theta = self.theta_kp + self.theta_ki * self.Ts / 2.0 + 2.0 * self.theta_kd / self.Ts
        self.A1theta = -self.theta_kp + self.theta_ki * self.Ts / 2.0 - 2.0 * self.theta_kd / self.Ts
        self.A2theta = self.theta_kd / self.Ts  # Additional term needed for proper derivative calculation
    ...
    def write_file(self, t, x, y, theta, x_sim, y_sim, theta_sim, e_x, e_y, e_theta, w1, w2, w3, w4):
        with open(self.file_name, "a") as file:
            file.write(f"{t}\t{x}\t{y}\t{theta}\t{x_sim}\t{y_sim}\t{theta_sim}\t{e_x}\t{e_y}\t{e_theta}\t{w1}\t{w2}\t{w3}\t{w4}")
            file.write("\n")
    ...
    def run(self):
        ...
        for i in range(0,len(self.goalx)):
            ...
            self.acx = self.acx + self.A0x * e_x + self.A1x * self.e_x_ant + self.A2x * (e_x - self.e_x_ant2)
            self.acy = self.acy + self.A0y * e_y + self.A1y * self.e_y_ant + self.A2y * (e_y - self.e_y_ant2)
            self.acw = self.acw + self.A0theta * e_theta + self.A1theta * self.e_theta_ant + self.A2theta * (e_theta - self.e_theta_ant2)
            
            
            self.e_x_ant2 = self.e_x_ant
            self.e_y_ant2 = self.e_y_ant
            self.e_theta_ant2 = self.e_theta_ant
