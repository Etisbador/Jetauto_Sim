#include <gazebo_mecanum_plugins/gazebo_ros_mecanum_drive_wc.h>
...
namespace gazebo
{
    ...
    void GazeboRosMecanumDrive::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
    {
        parent_ = parent;
        /* Parse parameters */
        std::cout<<" loaded modified WC plugin"<<"\n";
        ...
        //**********************
        command_wheel_topic_= "wheel_vel";
                if(!sdf->HasElement("commandWheelVelTopic"))
                {
                    ROS_WARN_NAMED("MecanumDiffDrive", "MecanumDiffDrivePlugin (ns = %s) missing <commandWheelVelTopic>, "
                                   "defaults to \"%s\"",
                                    robot_namespace_.c_str(), command_wheel_topic_.c_str());
                }
                else
                {
                	command_wheel_topic_ = sdf->GetElement("commandWheelVelTopic")->Get<std::string>();
                }
        //***********************
        ...
        //**********************************************
        wheel_speed_topic_ = "wheel_speed";
               if(!sdf->HasElement("wheelSpeedTopic"))
               {
                   ROS_WARN_NAMED("MecanumDiffDrive", "MecanumDiffDrivePlugin (ns = %s) missing <wheelSpeedTopic>, "
                                  "defaults to \"%s\"",
                                   robot_namespace_.c_str(), wheel_speed_topic_.c_str());
               }
               else
               {
            	   wheel_speed_topic_ = sdf->GetElement("wheelSpeedTopic")->Get<std::string>();
               }
        //***********************************************
        ...
        ros::SubscribeOptions so = ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
                                                                                       ...
                                                                                       ros::VoidPtr(), &queue_);
        //**
        ros::SubscribeOptions soW = ros::SubscribeOptions::create<std_msgs::Float32MultiArray>(command_wheel_topic_, 1,
                                                                                            boost::bind(&GazeboRosMecanumDrive::wheelVelCallback, this, _1),
                                                                                            ros::VoidPtr(), &queue_);
        vel_sub_ = rosnode_->subscribe(so);
        //**
        wheel_sub_ = rosnode_->subscribe(soW);
        odometry_pub_ = rosnode_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

        // wheel speed publisher
        wheel_speed_pub_ = rosnode_->advertise<std_msgs::Float32MultiArray>(wheel_speed_topic_, 1);
        ...
        if(odometry_rate_ > 0.0)
        {
            ...
            if(seconds_since_last_update > (1.0 / odometry_rate_))
            {
                ...
                if(!isRollerModel_)
                {
                    ...
                    double vx_w = cal_LineVel_.vel_x * cosf(yaw) - cal_LineVel_.vel_y * sinf(yaw);
                    double vy_w = cal_LineVel_.vel_y * cosf(yaw) + cal_LineVel_.vel_x * sinf(yaw);
                    double th_w = cal_LineVel_.vel_th;
    ...
    //****
    void GazeboRosMecanumDrive::wheelVelCallback(const std_msgs::Float32MultiArray::ConstPtr& wheel_msg)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
        wLR = wheel_msg->data[0];
        wLF = wheel_msg->data[1];
        wRF = wheel_msg->data[2];
        wRR = wheel_msg->data[3];
        //**
    ...
    {
    	if(use_wheel_speed){
    		 wheel_speed_[LEFT_REAR]   = wLR;
    		 wheel_speed_[LEFT_FRONT]  = wLF;
    		 wheel_speed_[RIGHT_FRONT] = wRF;
    		 wheel_speed_[RIGHT_REAR]  = wRR;
    	} else{
    ...
    // getting data for base_footprint to odom transform
        ...
        //******************************
       /* std_msgs::Float32MultiArray wvel;
        wvel.data.resize(4);
        wvel.data[0] = joints_[LEFT_REAR]->GetVelocity(0);
        wvel.data[1] = joints_[LEFT_FRONT]->GetVelocity(0);
        wvel.data[2] = joints_[RIGHT_FRONT]->GetVelocity(0);
        wvel.data[3] = joints_[RIGHT_REAR]->GetVelocity(0);
        wheel_speed_pub_.publish(wvel);*/
    }
    void GazeboRosMecanumDrive::publishWheelSpeed(){
    std_msgs::Float32MultiArray wvel;
        wvel.data.resize(4);
        wvel.data[0] = joints_[LEFT_REAR]->GetVelocity(0);
        wvel.data[1] = joints_[LEFT_FRONT]->GetVelocity(0);
    ...
    void GazeboRosMecanumDrive::calkinematics(linear_vel &line_vel)
    {
        ...
        std::cout<<"wheel speed: LR: "<<wheel_encoder[LEFT_REAR]<< " LF: "<<wheel_encoder[LEFT_FRONT]
                 <<" RF: "<<wheel_encoder[RIGHT_FRONT]<<" RR" <<wheel_encoder[RIGHT_REAR]<<"\n";
        ...
        std::cout<<"Vx: "<<line_vel.vel_x<< " Vy: "<<line_vel.vel_y
                         <<" w "<<line_vel.vel_th<<"\n";

        //double l = 1.0 / (2 * (wheel_separation_w + wheel_separation_l));
        /*double l = 1.0 / ((wheel_separation_w + wheel_separation_l));

        line_vel.vel_x = (wheel_encoder[LEFT_REAR] + wheel_encoder[LEFT_FRONT] + wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) * wheel_diameter_ / 8; //2;
        line_vel.vel_y = (wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] + wheel_encoder[RIGHT_FRONT] - wheel_encoder[RIGHT_REAR]) * wheel_diameter_ / 8;// /2;
        line_vel.vel_th = (-wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] + wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) * l * wheel_diameter_ / 8 ;//2;
